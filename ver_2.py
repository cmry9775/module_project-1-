...
OUTLIER_DAYS = {
    (
        5,
        5,
    ): "어린이날은 방문객이 예측치를 크게 벗어나는 이상치라, 별도 안내로 처리합니다.",
}
...
with st.sidebar:
    ...
    # 차트에서 선택한 날짜와 날씨가 표시될 자리
    selected_weather_area = st.container()
...
with left:
    ...
    bar_colors = [
        COLOR_WEEKEND if flag else COLOR_WEEKDAY for flag in df["주말/공휴일"]
    ]
    fig = go.Figure()
    fig.add_bar(
        x=df["라벨"],
    ...
    with summary_area:
        if sel_idx == 0:
            st.markdown(
                f"#### 📊 오늘 · {sel_date.strftime('%m월 %d일')} ({WEEKDAY_KR[sel_date.weekday()]}) 예측"
            )
        else:
            st.markdown(
                f"#### 📊 {sel_date.strftime('%m월 %d일')} ({WEEKDAY_KR[sel_date.weekday()]}) 예측"
            )
...
with selected_weather_area:
    st.markdown("### 🌤️ 선택한 날짜")
    st.write(
        f"{sel_date.strftime('%Y년 %m월 %d일')} "
        f"({WEEKDAY_KR[sel_date.weekday()]})"
    )

    st.metric("평균기온", f"{sel_weather['temp']}℃")
    st.metric("일강수량", f"{sel_weather['rainfall']}mm")
    st.metric("습도", f"{sel_weather['humidity']}%")
    st.metric("날씨 쾌적도", f"{weather_score(sel_weather):.0f} / 100")

    if sel_weather["rainfall"] > 20:
        st.warning("강수량이 많아 야외 활동에 불편할 수 있습니다.")

    elif sel_weather["rainfall"] > 0:
        st.info("비가 예상되므로 우산을 준비하는 것이 좋습니다.")

    elif sel_weather["temp"] >= 30:
        st.warning("기온이 높으므로 온열질환에 주의하세요.")

    else:
        st.success("야외 활동에 비교적 적합한 날씨입니다.")

    st.caption(
        f"{sel_weather['season']}"
        + (
            f" · {holiday_name(sel_date)}"
            if is_holiday(sel_date)
            else ""
        )
    )
...
# ---------------------------------------------------------------------------
# 5-3. 날씨·혼잡도 비율을 반영한 추천 날짜
# ---------------------------------------------------------------------------
weather_scores = []

for weather in weathers:
    weather_scores.append(weather_score(weather))

df["날씨 점수"] = weather_scores

# 방문객이 적을수록 한산도 점수가 높아지도록 0~100점으로 변환
min_visitors = df["예상 입장객"].min()
max_visitors = df["예상 입장객"].max()

if max_visitors == min_visitors:
    df["한산도 점수"] = 100
else:
    df["한산도 점수"] = (100- ((df["예상 입장객"] - min_visitors)/ (max_visitors - min_visitors)* 100))

# 슬라이더 값 0은 혼잡도 우선, 10은 날씨 우선
weather_ratio = weather_weight / 10
crowd_ratio = 1 - weather_ratio

df["추천 점수"] = (
    df["날씨 점수"] * weather_ratio
    + df["한산도 점수"] * crowd_ratio
)

# 추천 점수가 가장 높은 날짜 선택
best_index = df["추천 점수"].idxmax()
best_date = df.loc[best_index, "날짜"]
best_label = df.loc[best_index, "라벨"]
best_visitors = df.loc[best_index, "예상 입장객"]
best_weather = weathers[best_index]
best_score = df.loc[best_index, "추천 점수"]

with right:
    st.markdown("### ⭐ 가장 추천하는 방문 날짜")
    st.success(f"{best_label}을 가장 추천합니다.")

    st.metric("추천 날짜", best_date.strftime("%m월 %d일"))
    st.metric("예상 입장객 수", f"{best_visitors:,}명")
    st.metric("예상 기온", f"{best_weather['temp']}℃")
    st.metric("종합 추천 점수", f"{best_score:.1f} / 100")

    st.caption(
        f"날씨 {weather_weight * 10}% · "
        f"혼잡도 {(10 - weather_weight) * 10}%를 반영한 결과입니다."
    )
