...
def render_score_breakdown(row, *, compact: bool) -> None:
    weather_raw = float(row["weather_score"])
    visitor_raw = float(row["visitor_score"])
    weather_weighted = weather_raw * w_weather
    visitor_weighted = visitor_raw * w_visitor
    total = float(row["final_score"])
    ...
    with container:
        ...
        st.caption(
            f"날씨 {weather_weighted:.1f}점 + "
            f"한산함 {visitor_weighted:.1f}점"
        )
...
def render_trend(*, show_weather: bool) -> None:
    ...
    _suffix = (
        ", 오늘" if sel_date == today else (", 추천일" if sel_idx == best_idx else "")
    )
    st.markdown(
        f"""
    ...
    bar_colors = [
        hex_rgba(
            LEVEL_COLOR.get(lv, LEVEL_COLOR_FALLBACK), 1.0 if i == sel_idx else 0.55
        )
