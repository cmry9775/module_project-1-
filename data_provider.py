  ...
  {"days": [
      {
        ...
        "meta": {
          ...
          "forecast_time": str,
          "model_dayoff": bool,     # 그날이 쉬는 날인지
          "model_nextdayoff": bool, # 다음 날이 쉬는 날인지
          "model_month": str        # "08"
        }
      }, ...
  ],
  "notice": {                       # 날짜별이 아니라 10일 전체에 공통인 공지
    "status": "success",
    "major_summary": str,           # 공지를 종합한 1~2줄 브리핑
    "notices_list": [               # 중요도 상위 5건
      {"category": "축제",          # 축제/행사/점검/안내/예매
       "title": str,
       "date_string": "08.19 ~ 08.20",   # 기간이 없으면 "상시"
       "status_tag": "진행 중",          # "진행 중" | "D-Day" | "D-6"
       "url": str}, ...
    ],
    "event_board_url": str
  }}

[ UI 수신 스키마 ]  fetch_bundle()["rows"] 는 위 payload 의 days 를 아래 dict
                    리스트로 펴서 날짜 오름차순으로 반환한다.
...
[ 공지 수신 스키마 ]  fetch_bundle()["notice"] 는 payload 의 notice 를 아래로 옮긴다.
                     표시 문구는 받은 값을 그대로 쓰고, UI는 색상 키만 붙인다.

  ok        : bool  status == "success"
  summary   : str   major_summary (사이드바 상단 브리핑)
  board_url : str   '전체 보기' 링크
  items     : list  진행 중 → 예정 → 상시 순으로 정렬된 공지. 종료된 건은 뺀다.
      category    : str  theme.NOTICE_COLOR 의 키
      title       : str
      period_text : str  date_string 그대로 ("상시" 포함)
      state       : str  theme.NOTICE_STATE_COLOR 의 키
      state_label : str  status_tag 그대로 ("진행 중" / "D-6" / "상시")
      dday        : int  "D-6" 의 6. 그 외는 0
      url         : str
────────────────────────────────────────────────────────────
"""
...
# congestion 정수 → 표시 라벨.
# 구간을 나누는 인원 기준은 ML팀에서 임시로 잡은 값이라 앞으로 바뀔 수 있다.
# 그래서 UI는 인원수로 혼잡도를 다시 판정하지 않고, 받은 정수를 라벨로 옮기기만 한다.
# 분류 모델과 회귀 모델이 따로 학습되어 예측 인원과 혼잡도가 어긋나 보이는 날도
# 그대로 표시한다.
...
# 오월드 뉴스&공지 게시판 목록. payload 가 글별 주소나 행사 게시판 주소를 주지
# 못했을 때 링크가 빈칸으로 남지 않도록 여기로 보낸다.
NOTICE_BOARD_URL = (
...
# status_tag 에서 D-day 숫자를 뽑는다. AI가 "D-6", "D - 6" 처럼 흘려 써도 받는다.
_DDAY_PATTERN = re.compile(r"D\s*-\s*(\d+)")


# ---------------------------------------------------------------------------
# 공개 함수
# ---------------------------------------------------------------------------
def fetch_bundle(start_date: dt.date, n_days: int = 10) -> dict:
    """start_date 부터 n_days 일치 예측과 기간 공통 공지를 함께 반환한다.

      rows   : list[dict]  UI 수신 스키마 (파일 상단 참고)
      notice : dict        공지 수신 스키마 (파일 상단 참고)

    예측과 공지를 한 payload 에서 같이 꺼낸다. 따로 호출하면 실연동에서 공지
    크롤링과 AI 요약이 호출마다 다시 돌아간다.
    ...
    return {
        ...
        "notice": normalize_notice(payload),
    }
...
def normalize_notice(payload) -> dict:
    """payload 의 notice 항목을 공지 수신 스키마로 변환한다.

    기간·상태는 공지팀이 이미 문자열로 정리해 주므로 UI가 날짜를 다시 계산하지
    않는다. 정렬과 색상 구분에 필요한 만큼만 태그에서 읽어 낸다.
    """
    ...
    items = [
        row
        for row in (_notice_row(n) for n in notice.get("notices_list") or [])
        if row is not None and row["state"] != "ended"
    ]
    items.sort(key=lambda r: (_NOTICE_STATE_ORDER[r["state"]], r["dday"]))

    return {
        "ok": str(notice.get("status", "")).lower() == "success",
        "summary": str(notice.get("major_summary") or ""),
        "board_url": str(notice.get("event_board_url") or NOTICE_BOARD_URL),
        "items": items,
    }


def _notice_row(notice) -> dict | None:
    ...
    state, state_label, dday = _notice_state(notice.get("status_tag"))
    return {
        "category": str(notice.get("category") or "안내").strip() or "안내",
        "title": title,
        "period_text": str(notice.get("date_string") or "").strip() or "상시",
        "state": state,
        "state_label": state_label,
        "dday": dday,
        "url": str(notice.get("url") or "").strip() or NOTICE_BOARD_URL,
    }


def _notice_state(status_tag) -> tuple[str, str, int]:
    """status_tag 를 색상용 상태 키와 D-day 숫자로 나눈다.

    화면에 찍는 문구는 받은 태그를 그대로 쓴다. 모르는 태그는 상시로 두어
    목록에서 사라지지 않게 한다.
    """
    tag = " ".join(str(status_tag or "").split())

    if not tag or tag == "상시":
        return "always", "상시", 0
    if tag.startswith("진행") or tag.upper() in ("D-DAY", "DDAY"):
        return "ongoing", tag, 0

    matched = _DDAY_PATTERN.search(tag.upper())
    if matched:
        return "upcoming", tag, int(matched.group(1))
    if tag in ("종료", "마감"):
        return "ended", tag, 0
...
def _meta_value(meta: dict, name: str, default):
    ...
    """
    for key in (f"model_{name}", f"congestion_{name}"):
        if key in meta:
            return meta[key]
    return default
...
def _to_row(day) -> dict | None:
    ...
    return {
        ...
        "is_dayoff": bool(_meta_value(meta, "dayoff", date.weekday() >= 5)),
        "is_next_dayoff": bool(_meta_value(meta, "nextdayoff", False)),
        "month": str(_meta_value(meta, "month", "") or f"{date.month:02d}"),
    }
...
# 공지 mock. (분류, 제목, 시작일, 종료일) 이고 시작일이 None 이면 상시 안내다.
# 실제로는 공지 게시판을 크롤링해 AI가 골라 준 상위 5건이 온다.
_MOCK_NOTICES = [
    ("축제", "오월드 썸머 나이트 페스티벌", dt.date(2026, 8, 8), dt.date(2026, 8, 23)),
    ("행사", "아기사자 삼둥이 특별 공개", dt.date(2026, 8, 15), dt.date(2026, 8, 17)),
    ("점검", "회전목마·바이킹 정기 안전점검 운휴", dt.date(2026, 8, 19), dt.date(2026, 8, 20)),
    ("안내", "워터파크 여름 시즌 운영 종료", dt.date(2026, 8, 31), None),
    ("예매", "가을 국화 축제 사전 예매 시작", dt.date(2026, 9, 12), dt.date(2026, 10, 5)),
    ("안내", "여름철 운영시간 · 주차 안내", None, None),
    ("축제", "한여름 물총 대작전", dt.date(2026, 7, 18), dt.date(2026, 8, 2)),
]


def _mock_notice_item(category, title, start, end, today: dt.date) -> dict:
    """공지 한 건을 payload 형식(date_string/status_tag)으로 만든다."""
    if start is None:
        period, tag = "상시", "상시"
    else:
        end = end or start
...
def _mock_notice(today: dt.date) -> dict:
    items = [_mock_notice_item(*notice, today) for notice in _MOCK_NOTICES]
    return {
        "status": "success",
        "major_summary": (
            "야간 개장과 광복절 연휴가 겹치는 8월 중순 주말에 방문객이 몰릴 것으로 "
            "보이고, 일부 놀이기구는 안전점검으로 운휴합니다."
        ),
        "notices_list": [n for n in items if n["status_tag"] != "종료"][:5],
        "event_board_url": NOTICE_BOARD_URL,
    }
    ...
    for i in range(n_days):
        ...
        days.append({
            ...
            "meta": {
                ...
                "weather_source": "Mock",
                "forecast_time": "12:00",
                "model_dayoff": _mock_dayoff(d),
                "model_nextdayoff": _mock_dayoff(d + dt.timedelta(days=1)),
                "model_month": f"{d.month:02d}",
            },
        })
    return {"days": days, "notice": _mock_notice(start_date)}
